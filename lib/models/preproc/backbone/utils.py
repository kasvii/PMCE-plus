...
def draw_bbox(img_path, bbox, bbox_format='xyxy', output_path=None, color=(0, 255, 0), thickness=2):
    """
    Draw a bounding box on an image and save or display the result.

    Args:
        img_path (str): Path to the input image.
        bbox (list or np.array): Bounding box coordinates.
            - 'xyxy': [x_min, y_min, x_max, y_max]
            - 'xywh': [x, y, width, height]
        bbox_format (str): Format of the bounding box ('xyxy' or 'xywh').
    ...
    if bbox_format == 'xywh':
        ...
        raise ValueError(f"Unsupported bbox_format: {bbox_format}. Use 'xyxy' or 'xywh'.")
    
    # Draw the bounding box on the image
    x_min, y_min, x_max, y_max = map(int, [x_min, y_min, x_max, y_max])  # Ensure integers
