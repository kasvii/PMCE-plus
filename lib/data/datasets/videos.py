...
class Human36M(Dataset3D):
    def __init__(self, cfg, dset='train'):
        parsed_data_path = os.path.join(_C.PATHS.PARSED_DATA, f'human36m_vm_{dset}_tight_backbone.pth')
        ...
        self.has_3d = True
        self.has_traj = True
        self.has_smpl = True # False
        self.has_verts = True # False
...
class MPII3D(Dataset3D):
    def __init__(self, cfg, dset='train'):
        parsed_data_path = os.path.join(_C.PATHS.PARSED_DATA, f'mpii3d_vm_{dset}_backbone.pth')
        parsed_data_path = parsed_data_path.replace('backbone', cfg.MODEL.BACKBONE.lower())
...
class ThreeDPW(Dataset3D):
    def __init__(self, cfg, dset='train'):
        parsed_data_path = os.path.join(_C.PATHS.PARSED_DATA, f'3dpw_vm_{dset}_backbone.pth')
        parsed_data_path = parsed_data_path.replace('backbone', cfg.MODEL.BACKBONE.lower())
