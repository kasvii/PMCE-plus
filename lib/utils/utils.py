...
def get_optimizer(cfg, model, optim_type, momentum, stage):
    if stage == 'stage2':
        param_list = [{'params': model.parameters(), 'lr': cfg.TRAIN.LR_FINETUNE}]
    else:
...
def prepare_groundtruth(batch, device):
    groundtruths = dict()
    gt_keys = ['pose', 'cam', 'betas', 'kp3d', 'bbox', 'gt_marker']          # Evaluation
...
def prepare_input(batch, device, use_features):
    ...
    if 'pred_marker' in batch:
        marker = batch['pred_marker'].to(device).float()
    else:
        marker = None
