...
from lib.utils.utils import prepare_batch, save_obj
...
def main(cfg, args):
    ...
    J_regressor_wham = torch.from_numpy(np.load(_C.BMODEL.JOINTS_REGRESSOR_WHAM)).unsqueeze(0).float().to(cfg.DEVICE)
    ...
    with torch.no_grad():
        for i in range(len(eval_loader)):
            ...
            x, marker, inits, features, kwargs, gt = prepare_batch(batch, cfg.DEVICE)
            vis = batch['vis']
            
            if cfg.FLIP_EVAL and cfg.TRAIN.STAGE=='stage2':
                flipped_batch = eval_loader.dataset.load_data(i, True)
                f_x, f_marker, f_inits, f_features, f_kwargs, _ = prepare_batch(flipped_batch, cfg.DEVICE)
            ...
            target_output = smpl[batch['gender']](
                ...
                pose2rot=False)
            if cfg.TRAIN.STAGE == 'stage1':
                target_j3d = torch.matmul(J_regressor_wham, target_output.vertices).cpu()
            ...
            accel = compute_error_accel(joints_pred=pred_j3d, joints_gt=target_j3d, vis=vis)[1:-1]
            ...
            # Save path
            viz_pth = osp.join('output', 'visualization_3dpw')
            ...
            imname_list = sorted(glob(osp.join(_C.PATHS.THREEDPW_PTH, 'images', batch['vid'][:-2], '*.jpg')))
            ...
            for i, frame in enumerate(frame_list):
                ...
                print(vertices)
                save_obj(vertices.detach().cpu().numpy(), smpl['neutral'].faces, f'{viz_pth}/{frame}.obj')
