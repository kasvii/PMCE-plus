...
def main(cfg, args):
    ...
    # ========= Dataloaders ========= #
    eval_loader = setup_eval_dataloader(cfg, 'mpii3d', 'test', cfg.MODEL.BACKBONE)
    ...
    with torch.no_grad():
        for i in range(len(eval_loader)):
            ...
            accel = accel * (50 ** 2)       # per frame^s to per s^2
            # =======>
            ...
            # Save path
            viz_pth = osp.join('output', 'visualization_mpii3d')
            ...
            imname_list = sorted(glob(osp.join(_C.PATHS.MPII3D_PTH, 'mpi_inf_3dhp_test_set',
                                'TS' + str(int(batch['vid'].split('subj')[-1])), 'imageSequence', '*.jpg')))
