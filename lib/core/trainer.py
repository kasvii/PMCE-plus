...
class Trainer():
    def __init__(self, 
        ...
        if self.train_stage == 'stage2':
            self.evaluation_accumulators = dict.fromkeys(
                ['pred_j3d', 'target_j3d', 'pred_kp3d', 'target_kp3d', 'pve'])
        else:    
            self.evaluation_accumulators = dict.fromkeys(
    ...
    def train(self, ):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            start = time.time()
            if self.train_stage=='stage2':
                pred = self.network(x, marker, features, **kwargs) # 81 frame
            else:
                pred = self.network(x, features, **kwargs)
            ...
            if self.train_stage=='stage2':
                summary_string += f'| 2d: {kp_2d_loss.avg:.2f} | 3d: {kp_3d_loss.avg:.2f} '
    ...
    def validate(self, ):
        ...
        with torch.no_grad():
            for i, batch in enumerate(self.valid_loader): # seqlen = video_len
                ...
                smpl = build_body_model(self.device, batch_size=len(pred['kp3d_nn']), gender=batch['gender'][0])
                ...
                if self.train_stage=='stage2':
                    pred_kp3d = pred['kp3d_nn_2'].squeeze(0).cpu()
                    target_kp3d = root_centering(gt_output.joints)
                    target_kp3d = target_kp3d[:, :pred_kp3d.shape[-2], :].cpu()
                    pred_kp3d, target_kp3d = batch_align_by_pelvis_coco(
                        pred_kp3d, target_kp3d, [11, 12]
                    )
                    
                    pred_reg_j3d = torch.matmul(self.J_regressor_eval, pred['verts_cam']).cpu()
                    target_reg_j3d = torch.matmul(self.J_regressor_eval, gt_output.vertices).cpu()
                    pred_verts = pred['verts_cam'].cpu()
                    target_verts = gt_output.vertices.cpu()
                    pred_reg_j3d, target_reg_j3d, pred_verts, target_verts = batch_align_by_pelvis(
                        [pred_reg_j3d, target_reg_j3d, pred_verts, target_verts], [2, 3]
                    )
                    
                    self.evaluation_accumulators['pred_kp3d'].append(pred_kp3d.numpy())
                    self.evaluation_accumulators['target_kp3d'].append(target_kp3d.numpy())
                    self.evaluation_accumulators['pred_j3d'].append(pred_reg_j3d.numpy())
                    self.evaluation_accumulators['target_j3d'].append(target_reg_j3d.numpy())
    ...
    def evaluate(self, ):
        ...
        if 'pve' in self.evaluation_accumulators.keys():
